...
# Batch sentence-retention experiments for MMLU-Redux.
# For each subset, it extracts attention when needed, aggregates word-level
# attention by sentence, and retains the top-attention 10%, 20%, and 30%.
# Prerequisite:
...
experiment_finished() {
  ...
  for percentage in "${TOP_K_PERCENTAGES[@]}"; do
    if [[ ! -f "final_results/maintain_only_sentences/high_att/${MODEL_NAME}-${DATA_NAME}-${subset}-att-right-top_percentage${percentage}.json" ]]; then
      return 1
    fi
  done
...
echo "============================================================"
echo "Sentence-retention experiments"
...
for subset in "${SUBSETS[@]}"; do
  ...
  if [[ "${SKIP_EXISTING}" == "1" ]] && experiment_finished "${subset}"; then
    echo "[Skip] Sentence-retention results already exist: ${subset}"
    continue
  fi

  echo "[Run] Retain top-attention sentences: ${subset}"
  python3 6_maintain_sentences.py \
    --data_name "${DATA_NAME}" \
    --sub_set "${subset}" \
    --model_name "${MODEL_NAME}" \
    --model_path "${MODEL_PATH}"
...
echo "============================================================"
echo "All sentence-retention experiments finished."
echo "============================================================"
